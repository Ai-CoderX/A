...
// Auto clean function - clears all JSON files every 20 minutes
const autoClean = () => {
  setInterval(async () => {
    try {
      const files = ['contact.json', 'message.json', 'message_count.json', 'metadata.json'];
      
      // Clear main JSON files
      for (const file of files) {
        await writeJSON(file, []);
      }
      
      // Clear participant files (those ending with _participants.json)
...
// Start auto clean (won't run on startup, first clean after 20 minutes)
autoClean();
